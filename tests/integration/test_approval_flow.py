"""Testes do fluxo de aprovação (endpoints e bloqueio READONLY)."""
...
class TestApprovalEndpoints:
    """Endpoints de aprovação — comportamento esperado."""

    def test_list_approvals_accepts_get(self):
        """GET /api/v1/approvals/ pode retornar 200 (lista) ou 401."""
        r = client.get("/api/v1/approvals/")
        assert r.status_code in (200, 401, 403)

    def test_create_approval_requires_body(self):
...
class TestReadonlyModeReflectedInApi:
    """Resposta da API deve refletir READONLY_MODE quando disponível."""

    def test_approvals_list_may_include_readonly_flag(self):
